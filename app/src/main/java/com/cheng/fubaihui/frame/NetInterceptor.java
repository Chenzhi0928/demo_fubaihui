...
import com.cheng.fubaihui.frame.safe.AES;
...
public class NetInterceptor {
    ...
    public OkHttpClient getClientWithCache() {
        return new OkHttpClient.Builder()
                .cache(new Cache(new File(BaseApp.getApplication().getCacheDir(), "NetCache"), 10 * 1024 * 1024))
                .connectTimeout(10, TimeUnit.SECONDS)
                .readTimeout(10, TimeUnit.SECONDS)
                .writeTimeout(10, TimeUnit.SECONDS)
                .addInterceptor(getLogInterceptor())
...
//                .addInterceptor(new CommonParamsPostJsonInterceptor())
                .addInterceptor(cacheInterceptor(BaseApp.getAppContext()))
                .addNetworkInterceptor(cacheInterceptor(BaseApp.getAppContext()))
                .sslSocketFactory(NetTrustManager.getNetTrustManager().createSSLSocketFactory()).hostnameVerifier(new NetTrustManager.TrustAllHostnameVerifier())
                .build();
    }
            ...
            public Response intercept(Chain chain) throws IOException {
                ...
                if (!isNetworkConnected(context)) {
                    request = request.newBuilder()
                            ...
                            .build();
                    Log.e("睚眦", "没网读取缓存");
                    Toast.makeText(BaseApp.getAppContext(),"当前网络不可用",Toast.LENGTH_SHORT).show();
                }
